...
BASE_PATH = "/home/msc23dhruv/msc23dhruvvol1claim/"

# config = {"lr": tune.loguniform(5e-5, 1e-4), "batch_size": tune.choice([2, 4, 8])}
config = {"lr": [3e-5, 1e-5, 5e-5], "batch_size": [2, 4, 8]}


normal_train_df = pd.read_pickle("../data/pubMed/normal/train.df")
normal_test_df = pd.read_pickle("../data/pubMed/normal/test.df")
normal_val_df = pd.read_pickle("../data/pubMed/normal/val.df")
...
(
    ...
    bioRed_input_ids,
    bioRed_token_type_ids,
    bioRed_attention_mask,
    bioRed_labels,
    test_size=0.2,
    random_state=42,
)
...
def train_model(train_dataloader, val_dataloader, val_labels, model, optimizer, epochs):
    ...
    return span_f1, avg_train_loss, pre, recall
...
bioRed_input_ids_train_df = pad_sequence(
    [torch.tensor(seq) for seq in bioRed_input_ids_train_df],
    batch_first=True,
    padding_value=0.0,
).tolist()
bioRed_attention_mask_train_df = pad_sequence(
    [torch.tensor(seq) for seq in bioRed_attention_mask_train_df],
    batch_first=True,
    padding_value=0.0,
).tolist()
bioRed_labels_train_df = pad_sequence(
    [torch.tensor(seq) for seq in bioRed_labels_train_df],
    batch_first=True,
    padding_value=0.0,
).tolist()

bioRed_input_ids_test_df = pad_sequence(
    [torch.tensor(seq) for seq in bioRed_input_ids_test_df],
    batch_first=True,
    padding_value=0.0,
).tolist()
bioRed_attention_mask_test_df = pad_sequence(
    [torch.tensor(seq) for seq in bioRed_attention_mask_test_df],
    batch_first=True,
    padding_value=0.0,
).tolist()
bioRed_labels_test_df = pad_sequence(
    [torch.tensor(seq) for seq in bioRed_labels_test_df],
    batch_first=True,
    padding_value=0.0,
).tolist()
...
# Convert the data into PyTorch tensors
bioRed_input_ids_train_df = torch.tensor(bioRed_input_ids_train_df, dtype=torch.long)
bioRed_attention_mask_train_df = torch.tensor(
    bioRed_attention_mask_train_df, dtype=torch.long
)
bioRed_labels_train_df = torch.tensor(bioRed_labels_train_df, dtype=torch.long)

bioRed_input_ids_test_df = torch.tensor(bioRed_input_ids_test_df, dtype=torch.long)
bioRed_attention_mask_test_df = torch.tensor(
    bioRed_attention_mask_test_df, dtype=torch.long
)
bioRed_labels_test_df = torch.tensor(bioRed_labels_test_df, dtype=torch.long)
...
def findHyperParameters(
    ...
    if model:
        model_normal = torch.load(BASE_PATH + "model.pth")
    else:
    ...
    f1, loss, pre, recall = train_model(
        train_dataloader,
        val_dataloader,
        normal_val_df_label_ids,
        model_normal,
    ...
    return f1, loss, pre, recall


best_params, best_loss = None, 0
for params in ParameterGrid(config):
    f1, loss, pre, recall = findHyperParameters(
        params,
        bioRed_train_data,
        bioRed_train_sampler,
        bioRed_val_data,
        bioRed_val_sampler,
        bioRed_labels_test_df,
    )
...
train_dataloader = DataLoader(
    bioRed_train_data,
    sampler=bioRed_train_sampler,
    batch_size=best_params["batch_size"],
)
val_dataloader = DataLoader(
    bioRed_val_data,
    sampler=bioRed_val_sampler,
    batch_size=best_params["batch_size"],
)
...
torch.save(best_trained_model, BASE_PATH + "model.pth")
